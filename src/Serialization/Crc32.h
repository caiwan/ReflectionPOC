...
#else
	template <size_t N> constexpr Checksum Crc32Rec(const char (&str)[N], Checksum crc = 0xFFFFFFFF) noexcept
	{
		// TODO: Does the two produces the same result?
		crc = crc ^ 0xffffffffLU;
		for (const auto & chr : str) { crc = crcTable[(static_cast<Checksum>(crc) ^ (chr)) & 0xff] ^ (crc >> 8U); }
...
constexpr Grafkit::Utils::Checksum operator"" _crc32(const char * s, size_t len) noexcept { return Grafkit::Utils::Crc32Rec(s, 0xFFFFFFFF); }
